	...
	<div class="breadcrumbs">
		<div class="container">
			<ol class="breadcrumb breadcrumb1 animated wow slideInLeft" data-wow-delay=".5s">
				...
				<li class="active"><?php
				$sqlmcategory = "SELECT * FROM category WHERE catid='$_GET[catid]' AND sub_catid='0'";
				$qsqlmcategory = mysqli_query($con,$sqlmcategory);
				$rsmcategory = mysqli_fetch_array($qsqlmcategory);
				echo $rsmcategory['catgory_title'];
				?></li>
				<?php
				$sqlscategory = "SELECT * FROM category WHERE catid='$_GET[subcatid]'";
				$qsqlscategory = mysqli_query($con,$sqlscategory);
				$rsscategory = mysqli_fetch_array($qsqlscategory);
				if(mysqli_num_rows($qsqlscategory) == 1)
				{
				?>
				<li class="active"><?php echo $rsscategory['catgory_title']; ?></li>
				<?php
				}
				?>
			</ol>
...
if(isset($_GET['s']))
{
	$sqlproduct = $sqlproduct . " AND (product.prodname LIKE '%$_GET[s]%' or product.prodspecif LIKE '%$_GET[s]%' )";
}
					...
					<figure>
						<div class="snipcart-item block">
							<div class="snipcart-thumb">
								...
								<p style="height: 25px;"><?php echo $rsproduct['prodname']; ?></p>
								<h4>₹<?php 
								if($rsproduct['discount'] == 0)
								{
